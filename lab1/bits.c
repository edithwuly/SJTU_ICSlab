...
int divpwr2(int x, int n) {
  return (x+(((x>>31)&0x1)<<n)+~((x>>31)&0x1)+1)>>n;
}
/* 
...
int satAdd(int x, int y) {
  ...
  int z=x+y;
  int signZ=(z>>31)&0x1;
  int sign1=(!(signX^signY))&(signX^signZ);
  int sign=sign1<<31;
  int sign_=sign|(sign+~sign1+1);
  return (~sign_&z)|(sign_&(sign+~0+!signZ));
}
/* 
