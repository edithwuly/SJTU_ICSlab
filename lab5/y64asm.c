...
int add_symbol(char *name)
{
    ...
    symbol_t *q = (symbol_t *)malloc(sizeof(symbol_t));
    q->name = name;
    q->addr = vmaddr;
    q->next = symtab->next;
    symtab->next = q;
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
	...
	return PARSE_ERR;
    if (find_register(*ptr)==NULL)
	return PARSE_ERR;
    reg_t *temp=find_register(*ptr);
    *regid=temp->id;
    *ptr+=temp->namelen;
    return PARSE_REG;
}
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    /* set 'ptr' and 'name' */
    SKIP_BLANK(*ptr);
    if (IS_END(*ptr) || !IS_LETTER(*ptr)) 
	return PARSE_ERR;
    int i=0;
    for (i;IS_LETTER(*ptr+i)||IS_DIGIT(*ptr+i);i++);
...
parse_t parse_digit(char **ptr, long *value)
{
    ...
    /* set 'ptr' and 'value' */
    SKIP_BLANK(*ptr);
    if (IS_END(*ptr) || !IS_DIGIT(*ptr)) 
	return PARSE_ERR;
    int i=0;
    for (i;IS_DIGIT(*ptr+i)||IS_LETTER(*ptr+i)||*(*ptr+i)=='-';i++);
    if ((**ptr == '0' && *(*ptr+1)== 'x')||(**ptr=='-' && *(*ptr+1) == '0' && *(*ptr+2)== 'x' )){
	*value= strtoull(*ptr,NULL, 16);}
    else
...
parse_t parse_imm(char **ptr, char **name, long *value)
{
    ...
    SKIP_BLANK(*ptr);
    if (IS_END(*ptr)||(!IS_IMM(*ptr) && !IS_LETTER(*ptr))) 
	return PARSE_ERR;
    if (IS_IMM(*ptr)){
	*ptr+=1;
	if(!IS_DIGIT(*ptr)) 
...
parse_t parse_data(char **ptr, char **name, long *value)
{
    ...
    SKIP_BLANK(*ptr);
    if (IS_END(*ptr)||(!IS_DIGIT(*ptr) && !IS_LETTER(*ptr))) 
	return PARSE_ERR;
    if(IS_DIGIT(*ptr)){
	*name=NULL;
	return parse_digit(ptr,value);
...
*           call SUM  #invoke SUM function */
	...
	switch (HIGH(inst->code)){
		...
		regid_t temp;
		if (parse_reg(&ptr, &temp) == PARSE_ERR) {
		    line->type = TYPE_ERR;
		    err_print("Invalid REG");
		    return line->type; 
	    	}
		line->y64bin.codes[1] = HPACK(temp, 0xF);
		goto loop;
	    }
		...
		char *name=NULL;
		long value;
		parse_t temp=parse_imm(&ptr,&name,&value);
		if(temp==PARSE_ERR) {
		    line->type = TYPE_ERR;
		    err_print("Invalid Immediate");
		    return line->type;
		}
		if (temp==PARSE_SYMBOL)
		    add_reloc(name, &line->y64bin);
		long * tmp=(long *)&line->y64bin.codes[2];
		* tmp=value;
		if (parse_delim(&ptr, ',')== PARSE_ERR) {
		    line->type = TYPE_ERR;
		    err_print("Invalid ','");
		...
		* tmp=value;
		...
		switch (LOW(inst->code)) {
			...
			long *temp=(long *)&line->y64bin.codes[0];
	    		*temp=value;
			goto loop;
		    }
		    case D_POS: {
...
int relocate(void)
{
    ...
    while (rtmp) {
        ...
        /* next */
	symbol_t *temp = find_symbol(rtmp->name);
	if(!temp){
	...
	if(itype==I_IRMOVQ){
	    long *tmp=(long *)&rtmp->y64bin->codes[2];
	    *tmp=temp->addr;
	}
	if(itype==I_CALL||itype == I_JMP){
	    long *tmp=(long *)&rtmp->y64bin->codes[1];
	    *tmp=temp->addr;
	}
	if (itype==I_DIRECTIVE){
	    long *tmp=(long *)&rtmp->y64bin->codes[0];
	    *tmp=temp->addr;
	}
        rtmp = rtmp->next;
...
int binfile(FILE *out)
{
    ...
    line_t *p = line_head;
    int pos = 0;
    bool_t final(line_t *p){
	p = p->next;
	while(p){
	    if(p->y64bin.bytes != 0) return FALSE;
	    p = p->next;
	}
	return TRUE;
    }
    while(p){
    	if(pos < p->y64bin.addr && !final(p)){
    	    for(int i=0; i <p->y64bin.addr - pos; i++)
		fprintf(out, "%c", 0);
            pos = p->y64bin.addr;
        }
