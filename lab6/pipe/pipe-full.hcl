...
##1.a load forwarding of the combination of mrmovq/rmmovq
##2.a load forwarding of the combination of pushq/ret
##3.implementation of iaddq
##4.100% correctly predict of the combination of andq/jle
...
word f_pc = [
	D_icode == IJXX && f_ifun == 1 && E_icode == IOPQ && E_ifun == 2 && E_srcA == E_srcB && E_valA > 0 : D_valP;
	# Mispredicted branch.  Fetch at incremented PC
	M_icode == IJXX && !M_Cnd && !(M_ifun == 1 && W_icode == IOPQ ) : M_valA;
	# Completion of RET instruction
	W_icode == IRET : W_valM;
	E_icode == IRET && M_icode == IPUSHQ : M_valA;
...
bool F_stall =
	...
	(IRET in { D_icode, E_icode, M_icode } && !((D_icode == IRET && E_icode == IPUSHQ) || (E_icode == IRET && M_icode == IPUSHQ) || (M_icode == IRET && W_icode == IPUSHQ)));
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd && !(E_ifun == 1 && M_icode == IOPQ && M_ifun == 2 && M_valE > 0)) ||
	# Stalling at fetch while ret passes through pipeline
	# but not condition for a load/use hazard
	!(E_icode in { IMRMOVQ, IPOPQ } && E_dstM in { d_srcA, d_srcB } && !D_icode in { IRMMOVQ }) && (IRET in { D_icode, E_icode, M_icode } && !((D_icode == IRET && E_icode == IPUSHQ) || (E_icode == IRET && M_icode == IPUSHQ) || (M_icode == IRET && W_icode == IPUSHQ)));
...
bool E_bubble =
	...
	# Conditions for a load/use hazard
	E_icode in { IMRMOVQ, IPOPQ } &&
	 E_dstM in { d_srcA, d_srcB} && !D_icode in { IRMMOVQ };
