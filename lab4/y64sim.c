...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    bool_t ovf = FALSE;
    if (op == A_ADD && ((argA > 0 && argB > 0 && val <=0)||(argA < 0 && argB < 0 && val >=0)))
	ovf = TRUE;
    if (op == A_SUB && ((argA>0 && argB<0 && val >=0)||(argA<0 && argB>0 && val<=0)))
	ovf = TRUE;
...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
	...
	break;
      case(C_LE):
	if (((GET_SF(cc)^GET_OF(cc))||GET_ZF(cc)) == 1)
	    doit=TRUE;
	    break;
      case(C_L):
	if ((GET_SF(cc)^GET_OF(cc))==1)
	    doit=TRUE;
	    break;
      case(C_E):
	if (GET_ZF(cc)==1)
	...
	if (GET_ZF(cc)==0)
	    ...
	    break;
      case(C_GE):
	if ((GET_SF(cc) ^ GET_ZF(cc))==1){
	    doit=TRUE;
	    break;}
      case(C_G):
	if (((~(GET_SF(cc) ^ GET_OF(cc))&~GET_ZF(cc))&0x1)==1)
	    doit=TRUE;
	    break;
    }
    return doit;
...
stat_t nexti(y64sim_t *sim)
{
    ...
    long_t temp = 0, temp1 = 0, temp2 = 0, temp3 = 0, temp4 = 0;
	...
	if (cond_doit(sim->cc,ifun)){
	    set_reg_val(sim->r, GET_REGB(reg), get_reg_val(sim->r, GET_REGA(reg)));}
	sim->pc = next_pc;
	break;
      case I_IRMOVQ: /* 3:0 F:regB imm */
	set_reg_val(sim->r, GET_REGB(reg), imm);
	sim->pc = next_pc;
	break;
      case I_RMMOVQ: /* 4:0 regA:regB imm */
	set_long_val(sim->m, get_reg_val(sim->r, GET_REGB(reg))+imm, get_reg_val(sim->r, GET_REGA(reg)));
	sim->pc = next_pc;
	break;
      case I_MRMOVQ: /* 5:0 regB:regA imm */
	if (!get_long_val(sim->m, get_reg_val(sim->r, GET_REGB(reg))+imm, &temp)){
	    err_print("PC = 0x%lx, Invalid data address 0x%lx", sim-> pc, get_reg_val(sim->r, GET_REGB(reg))+imm);
            return STAT_ADR;   
	} 
	set_reg_val(sim->r, GET_REGA(reg), temp);
	sim->pc = next_pc;
	break;
      case I_ALU: /* 6:x regA:regB */
	temp4=compute_alu(ifun, get_reg_val(sim->r, GET_REGA(reg)), get_reg_val(sim->r, GET_REGB(reg)));
	sim->cc=compute_cc(ifun, get_reg_val(sim->r, GET_REGA(reg)), get_reg_val(sim->r, GET_REGB(reg)), temp4);
	set_reg_val(sim->r, GET_REGB(reg), temp4);
	sim->pc = next_pc; 
	break;
      case I_JMP: /* 7:x imm */
	sim->pc=cond_doit(sim->cc,ifun)?imm:next_pc;
	break;
      case I_CALL: /* 8:x imm */
	set_reg_val(sim->r, REG_RSP, get_reg_val(sim->r, REG_RSP)-8);
	if (get_reg_val(sim->r, REG_RSP)>=0xfffffffffffffff8){
	    err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim-> pc, get_reg_val(sim->r, REG_RSP));
	    return STAT_ADR;
	}
	set_long_val(sim->m, get_reg_val(sim->r, REG_RSP), next_pc);
	sim->pc = imm;
	break;
      case I_RET: /* 9:0 */
	if (!get_long_val(sim->m, get_reg_val(sim->r, REG_RSP), &temp2)){
	    err_print("PC = 0x%lx, Invalid data address 0x%lx", sim-> pc, get_reg_val(sim->r, REG_RSP));
	return STAT_ADR;   
	} 
	set_reg_val(sim->r, REG_RSP, get_reg_val(sim->r, REG_RSP)+8);
	if (get_reg_val(sim->r, REG_RSP)<0x8){
	    err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim-> pc, get_reg_val(sim->r, REG_RSP));
	    return STAT_ADR;
	}
	sim->pc = temp2;
	...
	temp3=get_reg_val(sim->r, GET_REGA(reg));
	set_reg_val(sim->r, REG_RSP, get_reg_val(sim->r, REG_RSP)-8);
	if (get_reg_val(sim->r, REG_RSP)>=0xfffffffffffffff8){
	    err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim-> pc, get_reg_val(sim->r, REG_RSP));
	    return STAT_ADR;
	}
	set_long_val(sim->m, get_reg_val(sim->r, REG_RSP), temp3);
	sim->pc = next_pc;
	break;
      case I_POPQ: /* B:0 regA:F */
	if (!get_long_val(sim->m, get_reg_val(sim->r, REG_RSP), &temp1)){
	    err_print("PC = 0x%lx, Invalid data address 0x%lx", sim-> pc, get_reg_val(sim->r, REG_RSP));
	return STAT_ADR;   
	} 
	set_reg_val(sim->r, REG_RSP, get_reg_val(sim->r, REG_RSP)+8);
	...
	set_reg_val(sim->r, GET_REGA(reg), temp1);
	sim->pc = next_pc;
    	break;
      default:
